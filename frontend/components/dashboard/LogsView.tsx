  ...
  return (
    <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm flex flex-col">
      <div className="px-4 py-3 border-b border-white/[0.05] flex items-center justify-between">
        <span className="text-[10px] font-semibold text-white tracking-widest uppercase">Session Logs</span>
        <span className="font-mono text-[11px] text-white">{logs.length} entries</span>
      </div>

      {logs.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-40 gap-3">
          <ClockIcon size={22} className="text-white" />
          <p className="text-[11px] text-white">No log entries yet.</p>
        </div>
      ) : (
        <div className="p-4 flex flex-col gap-1 font-mono text-[11px]">
          {logs.map((entry) => (
            <div key={entry.id} className="flex items-start gap-3 py-1.5 border-b border-white/[0.03] last:border-0">
              <span className="text-white shrink-0 w-16">{entry.timestamp}</span>
