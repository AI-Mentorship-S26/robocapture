    ...
    <>
      ...
      {uploadStatus !== "idle" && (
        <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-[11px] font-mono ${
          uploadStatus === "uploading" ? "border-blue-500/20 bg-blue-500/5 text-blue-400" :
          uploadStatus === "saved"     ? "border-emerald-500/20 bg-emerald-500/5 text-emerald-400" :
          ...
          {uploadStatus === "uploading" ? "Saving to Supabase…" :
           uploadStatus === "saved"     ? "Image saved to Supabase Storage" :
                                         "Save failed — check Supabase bucket"}
        </div>
      )}
